...
class EscrowService:
    ...
    def create_transaction(
        ...
        with transaction.atomic():
            try:
                ...
                logger.info(f"Created escrow transaction {escrow_transaction.id} for {amount} {currency}")
        ...
        try:
            with transaction.atomic():
                ...
                if escrow_transaction.status != 'PENDING':
                    logger.warning(f"Payment received for transaction {transaction_id} in invalid state: {escrow_transaction.status}")
                    return False
            ...
            logger.error(f"Failed to process payment for transaction {transaction_id}: {str(e)}")
            return False
        ...
        try:
            with transaction.atomic():
                ...
                if escrow_transaction.status != 'FUNDED':
                    logger.warning(f"Cannot start transfer for transaction {transaction_id} in state: {escrow_transaction.status}")
                    return False
                ...
                AuditLog.objects.create(
                    ...
                    details={
                        'transfer_deadline': escrow_transaction.transfer_deadline.isoformat() if escrow_transaction.transfer_deadline else None
                    }
                )
                
                logger.info(f"Transfer process started for transaction {transaction_id}")
            ...
            logger.error(f"Failed to start transfer process for transaction {transaction_id}: {str(e)}")
            return False
    
    @staticmethod
    def complete_transaction(
        transaction_id: uuid.UUID,
        verification_details: Dict[Any, Any]
    ) -> bool:
        """
        ...
        try:
            with transaction.atomic():
                ...
                if escrow_transaction.status not in ['VERIFYING', 'AWAITING_TRANSFER']:
                    logger.warning(f"Cannot complete transaction {transaction_id} in state: {escrow_transaction.status}")
                    return False
        ...
        try:
            with transaction.atomic():
                ...
                if escrow_transaction.status in ['COMPLETED', 'REFUNDED']:
                    logger.warning(f"Cannot refund transaction {transaction_id} in state: {escrow_transaction.status}")
                    return False
    ...
    def create_dispute(
        ...
        try:
            with transaction.atomic():
                ...
                if opened_by not in [escrow_transaction.buyer, escrow_transaction.seller]:
                    logger.warning(f"User {opened_by.telegram_id} not authorized to dispute transaction {transaction_id}")
                    return None
                ...
                logger.info(f"Dispute created for transaction {transaction_id} by user {opened_by.telegram_id}")
            ...
            logger.error(f"Failed to create dispute for transaction {transaction_id}: {str(e)}")
            return None
    ...
    def get_transaction_status(transaction_id: uuid.UUID) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            recent_logs = list(escrow_transaction.audit_logs.all()[:5].values(
            ...
            status_info = {
                ...
                'currency': escrow_transaction.currency,
                'usd_equivalent': str(escrow_transaction.usd_equivalent) if escrow_transaction.usd_equivalent else None,
                'buyer': {
                ...
                'timeline': {
                    ...
                    'funded_at': escrow_transaction.funded_at.isoformat() if escrow_transaction.funded_at else None,
                    'completed_at': escrow_transaction.completed_at.isoformat() if escrow_transaction.completed_at else None,
                    'transfer_deadline': escrow_transaction.transfer_deadline.isoformat() if escrow_transaction.transfer_deadline else None,
            ...
            if hasattr(escrow_transaction, 'disputecase'):
                ...
                status_info['dispute'] = {
                    ...
                    'opened_by': dispute.opened_by.username or str(dispute.opened_by.telegram_id),
            ...
            logger.error(f"Failed to get status for transaction {transaction_id}: {str(e)}")
            return None
    ...
    def get_user_transactions(
        ...
        try:
            ...
            transactions = EscrowTransaction.objects.filter(query).select_related(
                'buyer', 'seller', 'group_listing'
            ...
            for txn in transactions:
                ...
                result.append({
                    ...
                    'other_party': other_party.username or str(other_party.telegram_id),
            ...
            logger.error(f"Failed to get transactions for user {user.telegram_id}: {str(e)}")
            return []

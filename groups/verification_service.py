...
class GroupVerificationService:
    ...
    async def perform_full_verification(self, listing: GroupListing, transaction: EscrowTransaction = None) -> GroupVerificationResult:
        """
        ...
        try:
            ...
            return await self._create_failed_verification_result(transaction, listing, ["Failed to connect to Telegram API."])
        ...
        bot_status_ok, bot_details = self._verify_bot_status(admins)

        # Consolidate results
        all_checks_passed = ownership_ok and metadata_ok and bot_status_ok
        failure_reasons = [details for ok, details in [(ownership_ok, owner_details), (metadata_ok, metadata_details), (bot_status_ok, bot_details)] if not ok]
    ...
    def _verify_ownership(self, listing: GroupListing, creator: Any) -> Tuple[bool, str]:
        """
        ...
        if creator.user.id != listing.seller.telegram_id:
            return False, f"Seller ({listing.seller.telegram_id}) is not the group creator ({creator.user.id})."
    ...
    def _verify_metadata(self, listing: GroupListing, chat: Any) -> Tuple[bool, str]:
        ...
        if listing.group_title.lower() != chat.title.lower():
            return False, f"Group title mismatch. Expected '{listing.group_title}', found '{chat.title}'."
        return True, "Group metadata matches the listing."

    def _verify_bot_status(self, admins: list) -> Tuple[bool, str]:
        """
        Verifies that the bot is an admin in the group.
        """
        bot_user = self.bot.get_me()
        bot_admin = next((admin for admin in admins if admin.user.id == bot_user.id), None)
        if not bot_admin:
            return False, "The Trustlink bot is not an administrator in the group."
        return True, "Bot is confirmed as an administrator."

    @sync_to_async
    def _save_verification_result(self, transaction, listing, passed, **kwargs) -> GroupVerificationResult:
        """
        ...
        if transaction:
            ...
            logger.warning("Verification outside of a transaction is not fully implemented.")
            return None
            
        return result

    @sync_to_async
    def _create_failed_verification_result(self, transaction, listing, reasons: list) -> GroupVerificationResult:
        """
